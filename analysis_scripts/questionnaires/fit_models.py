...
from analysis_scripts.questionnaires.utils import (
    filter_condition,
    create_file_to_store,
    write_reg_results_to_file,
)
from analysis_scripts.questionnaires.visualize import (
    generate_diagnostic_plots,
    plot_participants_lines,
)


def get_regression_line(df, condition, range_max, path_to_store, seed=42):
    ...
    mdf_no_random_slopes_zpdes = smf.mixedlm(
        f"{condition} ~ session_id", zpdes, groups=zpdes["id_participant"]
    ).fit()
    mdf_no_random_slopes_baseline = smf.mixedlm(
        f"{condition} ~ session_id", baseline, groups=baseline["id_participant"]
    ).fit()
    zpdes_p = (
        mdf_no_random_slopes_zpdes.params["Intercept"],
        mdf_no_random_slopes_zpdes.params["session_id"],
    )
    baseline_p = (
        mdf_no_random_slopes_baseline.params["Intercept"],
        mdf_no_random_slopes_baseline.params["session_id"],
    )
    zpdes["group"] = "zpdes"
    ...
    mdf_combined = smf.mixedlm(
        model_formula, combined_data, groups=combined_data["id_participant"]
    ).fit()
    shapiro_test = stats.shapiro(mdf_combined.resid)
    info = (
        str(mdf_combined.summary())
        + f"\n Shapiro test: {shapiro_test.statistic}, p={shapiro_test.pvalue} \n"
    )
    info_bayesian = (
        f"\n Bayesian analysis {condition}: \n"
        + get_bayesian_mixed_lm_model(
            combined_data, condition, path_to_store, range_max=range_max, seed=seed
        )
    )
...
def get_bayesian_mixed_lm_model(data, condition, path_to_store, range_max=20, seed=42):
    # Build model
    model = build_model(data, range_max, condition)
    # Then get MCMC estimations
    with model:
        # Inference
        trace = pm.sample(10000, tune=1000, target_accept=0.95, seed=seed)
...
def get_advanced_scatter_plots(
    questionnaire_name,
    study,
    conditions,
    xtickslabels,
    ...
    # First retrieve df:
    df = pd.read_csv(
        f"data/{study}/questionnaires/{questionnaire_name}/{questionnaire_name}.csv"
    )
    ...
    for condition in conditions:
        ...
        zpdes_p, baseline_p, info_stats, info_bayesian = get_regression_line(
            ...
            condition,
            range_max=np.max(possible_values),
        ...
        write_reg_results_to_file(
            file_name, file_name_bayesian, info_stats, info_bayesian
        )
        plot_participants_lines(
            dict_groups,
            condition,
