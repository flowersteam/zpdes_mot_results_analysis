...
from code.questionnaires.preprocessing import treat_LP, treat_UES, treat_TENS, treat_SIMS, treat_nasa_tlx
from code.questionnaires.visualize import descriptive_visualization, get_proportion_plots
from code.questionnaires.fit_models import get_advanced_scatter_plots
...
def run_all_descriptive_visualizations(study):
    with open('code/questionnaires/config/config_questionnaire.JSON', 'r') as f:
...
def run_mixed_effect_models(study):
    ...
    #
    # # SIMS
    # conditions = conditions['sims'][:-1]
    # possible_values = [i for i in range(0, 7)]
    # get_advanced_scatter_plots(questionnaire_name='sims', study=study, conditions=conditions,
    #                            xtickslabels=["s1", "s4", "s5", "s8"],
    #                            possible_values=possible_values, yticks=possible_values, ytickslabels=possible_values)
    # possible_values_sdi = [i for i in range(-16, 17)]
    # get_advanced_scatter_plots(questionnaire_name='lp', study=study, conditions=["SDI"],
    #                            xtickslabels=["s1", "s4", "s5", "s8"],
    ...
    get_advanced_scatter_plots(questionnaire_name='lp', study=study, conditions=conditions['tens'],
                               xtickslabels=["s1", "s4", "s5", "s8"],
                               possible_values=possible_values, yticks=possible_values, ytickslabels=possible_values)


def run_questionnaires_analysis(study):
    chrono = Chrono()
...
if __name__ == '__main__':
    study = 'v3_prolific'
    run_questionnaires_analysis(study)
