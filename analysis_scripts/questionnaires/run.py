...
from analysis_scripts.questionnaires.preprocessing import (
    treat_LP,
    treat_UES,
    treat_TENS,
    treat_SIMS,
...
def run_all_descriptive_visualizations(study):
    with open(
        "analysis_scripts/questionnaires/config/config_questionnaire.JSON", "r"
    ) as f:
    ...
    get_proportion_plots(
        ...
        colors_barplots=colors_barplots,
    ...
    get_proportion_plots(
        ...
        ytickslabels=[i * 10 for i in range(11)],
        colors_barplots=colors_barplots,
        legend_labels=possible_values,
    )


def run_mixed_effect_models(study, seed=42):
    """
    ...
    get_advanced_scatter_plots(
        ...
        possible_values=possible_values,
        yticks=possible_values,
        ytickslabels=possible_values,
        seed=seed,
    )
    get_advanced_scatter_plots(
        questionnaire_name="nasa_tlx",
        ...
        xtickslabels=[f"s{i}" for i in range(1, 9)],
        possible_values=[i for i in range(0, 121, 10)],
        yticks=[i for i in range(0, 121, 10)],
    ...
    possible_values = [i for i in range(2, 8)]
    get_advanced_scatter_plots(
        questionnaire_name="lp",
        study=study,
        conditions=conditions["lp"],
        xtickslabels=["s1", "s4", "s5", "s8"],
        possible_values=possible_values,
        yticks=possible_values,
        ytickslabels=possible_values,
    ...
    get_advanced_scatter_plots(
        ...
        conditions=["SDI"],
        xtickslabels=["s1", "s4", "s5", "s8"],
        possible_values=possible_values_sdi,
        yticks=possible_values_sdi,
...
def run_questionnaires_analysis(studies):
    chrono = Chrono()
    print(
        "=====================Start [Questionnaires] - full analysis====================="
    )
