...
def run_broad_aggregate_difference(
    tasks, study, kept_columns, tasks_nb, expe_name, normalization_type="zscore"
):
    ...
    diff_zpdes, diff_baseline, diff_pre, diff_post = compute_diff(
        df, tasks, path_to_store
    )
...
def compute_diff(df, tasks, path_to_save):
    # Split into pre-post
    pre, post = (
        df[df["task_status"] == "PRE_TEST"],
        df[df["task_status"] == "POST_TEST"],
    )
    # Split into baseline-zpdes
    zpdes_pre, zpdes_post = (
        pre[pre["condition"] == "zpdes"][tasks + ["participant_id"]],
        post[post["condition"] == "zpdes"][tasks + ["participant_id"]],
    )
    baseline_pre, baseline_post = (
        pre[pre["condition"] == "baseline"][tasks + ["participant_id"]],
        post[post["condition"] == "baseline"][tasks + ["participant_id"]],
    )
    zpdes_pre, zpdes_post = (
        zpdes_pre.set_index("participant_id"),
        zpdes_post.set_index("participant_id"),
    )
    baseline_pre, baseline_post = (
        baseline_pre.set_index("participant_id"),
        baseline_post.set_index("participant_id"),
    )
    # Compute the mean z-score for each participant
    mean_pre_zpdes, mean_post_zpdes = (
        zpdes_pre.mean(axis=1).astype(float),
        zpdes_post.mean(axis=1).astype(float),
    )
    mean_pre_baseline, mean_post_baseline = (
        baseline_pre.mean(axis=1).astype(float),
        baseline_post.mean(axis=1).astype(float),
    )
    detect_baseline_moderators(
        mean_post_zpdes,
        mean_pre_zpdes,
        mean_post_baseline,
        mean_pre_baseline,
    ...
    diff_pre = ttest(
        pd.to_numeric(mean_pre_zpdes, errors="coerce"),
        pd.to_numeric(mean_pre_baseline, errors="coerce"),
        paired=False,
    )
    diff_post = ttest(
        pd.to_numeric(mean_post_zpdes, errors="coerce"),
        pd.to_numeric(mean_post_baseline, errors="coerce"),
        paired=False,
    )
...
def save_aggregate(
    ...
    save = pd.DataFrame.from_dict(
        {
            "t_diff_zpdes": [diff_zpdes["T"].values[0]],
...
def detect_baseline_moderators(
    mean_post_zpdes, mean_pre_zpdes, mean_post_baseline, mean_pre_baseline, path_to_save
):
    # Calculate changes
    change_zpdes = mean_post_zpdes - mean_pre_zpdes
    change_baseline = mean_post_baseline - mean_pre_baseline
    ...
    plt.savefig(f"{path_to_save}/zpdes_change.png")
    plt.close()
    ...
    plt.xlabel("Pre-test Mean Score")
    plt.ylabel("Change (Post-test Mean Score - Pre-test Mean Score)")
    plt.savefig(f"{path_to_save}/baseline_change.png")
    plt.close()
    ...
    data = pd.DataFrame(
        {
            ...
            "condition": ["zpdes"] * len(mean_pre_zpdes)
            + ["baseline"] * len(mean_pre_baseline),
        }
    )
    ...
    with open(f"{path_to_save}/mixedlm_summary.txt", "w", encoding="utf-8") as f:
