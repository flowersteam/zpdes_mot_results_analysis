...
def build_model(type, data, task_condition=None, coords=None):
    models = {
        "hierarbinom": build_hierar_binom,
...
def build_precision_moteval_hyper(data_combined, task_condition, coords):
    with pm.Model(coords=coords) as precision_model:
        ...
        # Data
        g = pm.Data("g", data_combined.condition_idx, dims="obs_id")
        t = pm.Data("t", data_combined.time_idx, dims="obs_id")
        ...
        if len(coords["condition"]) > 1:
            delta_zpdes = pm.Deterministic(
                "delta_zpdes", mu[1, 1] - mu[1, 0]
            )  # post - pre for zpdes
            delta_baseline = pm.Deterministic(
                "delta_baseline", mu[0, 1] - mu[0, 0]
            )  # post - pre for baseline
            delta_groups = pm.Deterministic(
                "delta_groups", delta_zpdes - delta_baseline
            )
        # Else, i.e control:
        else:
        ...
        pm.Normal(
            ...
            mu=mu[g, t],
            sigma=sigma[g, t],
            observed=data_combined[f"{task_condition}-accuracy_continuous"],
            dims="obs_id",
        )
...
def build_hierar_binom(data_combined, task_condition, coords):
    ...
    with pm.Model(coords=coords) as hierarchical_model:
        ...
        if len(coords["condition"]) > 1:
            delta_zpdes = pm.Deterministic(
                "delta_zpdes", beta[1, 1] - beta[1, 0]
            )  # post - pre for zpdes
            delta_baseline = pm.Deterministic(
                "delta_baseline", beta[0, 1] - beta[0, 0]
            )  # post - pre for baseline
            delta_groups = pm.Deterministic(
                "delta_groups", delta_zpdes - delta_baseline
            )
        ...
        p_observation = pm.Beta(
            "p_observation", mu=beta[g, t], sigma=sigma[g, t], dims="obs_id"
        )
        # Likelihood
        pm.Binomial(
            "y",
            n=n,
            p=p_observation,
            observed=data_combined[f"{task_condition}-correct"].values,
            dims="obs_id",
        )
...
def build_hierar_binom_gain(data_combined, task_condition, coords):
    ...
    with pm.Model(coords=coords) as hierarchical_model:
        ...
        if len(coords["condition"]) > 1:
            delta_zpdes = pm.Deterministic(
                "delta_zpdes", (beta[1, 1] - beta[1, 0]) / (1 - beta[1, 0])
            )  # post - pre for zpdes
            delta_baseline = pm.Deterministic(
                "delta_baseline", (beta[0, 1] - beta[0, 0]) / (1 - beta[0, 0])
            )  # post - pre for baseline
            delta_groups = pm.Deterministic(
                "delta_groups", delta_zpdes - delta_baseline
            )
    ...
    with pm.Model(coords=coords) as hierarchical_model:
        ...
        if len(coords["condition"]) > 1:
            ...
            delta = pm.Deterministic(
                "delta", beta[0, 1] - beta[0, 0]
            )  # post - pre for ctrl
        # Data
        g = pm.Data("g", data_combined.condition_idx, dims="obs_id")
        t = pm.Data("t", data_combined.time_idx, dims="obs_id")
        n = pm.Data("n", data_combined[f"{task_condition}-nb"], dims="obs_id")
        # Introduce a variability term for the local-level
        sigma = pm.HalfNormal("sigma", 0.1, dims=("condition", "time"))
        # Model local-level p's using the global-level p's as mean and sigma as SD
        p_observation = pm.Beta(
...
def build_hierar_binom_covar_pre(data_combined, task_condition, coords):
    with pm.Model(coords=coords) as hierarchical_model:
        ...
        acc_pre = data_combined.groupby("participant_id")[
            f"{task_condition}-accuracy"
        ].transform("first")
        pretest_accuracy = pm.Data("pretest_accuracy", acc_pre, dims="obs_id")
        ...
        p_mean = pm.Deterministic(
            "p_mean", pm.math.invlogit(p_mean)
        )  # Ensure it's between 0 and 1
        p_observation = pm.Beta(
            "p_observation", mu=p_mean, sigma=sigma[g, t], dims="obs_id"
        )
...
def build_RT_normal_hyper(data_combined, task_condition, coords):
    with pm.Model(coords=coords) as RT_model:
        ...
        pm.Normal(
            ...
            mu=mu[g, t],
            sigma=sigma[g, t],
            observed=data_combined[f"{task_condition}-rt"],
            dims="obs_id",
        )
    return RT_model
...
def build_ufov_threshold_hyper(data_combined, task_condition, coords):
    ...
    with pm.Model(coords=coords) as UFOV_model:
        ...
        mu = pm.Gamma(
            "mu", alpha=alpha_hyper, beta=beta_hyper, dims=("condition", "time")
        )
        ...
        pm.Normal(
            ...
            mu=mu[g, t],
            sigma=sigma[g, t],
            observed=data_combined[f"{task_condition}-threshold"],
            dims="obs_id",
        )
    return UFOV_model
...
def build_switching_cost_RT(data_combined, task_condition, coords):
    data_combined["switch_indicator"] = data_combined.apply(
        lambda row: literal_eval(row["switch_indicator"]), axis=1
    )
    data_combined["results_rt"] = data_combined.apply(
        lambda row: literal_eval(row["results_rt"]), axis=1
    )
    ...
    with pm.Model(coords=coords) as switching_cost_RT:
        # Data
        g = pm.Data("g", df_transformed.condition_idx, dims="obs_id")
        t = pm.Data("t", df_transformed.time_idx, dims="obs_id")
        s = pm.Data("s", df_transformed.switch_indicator, dims="obs_id")
        ...
        mu = pm.Normal(
            "mu", mu=0, sigma=10, dims=("condition", "time", "switch_indicator")
        )
        # Priors for group standard deviations given the hyperpriors
        sigma = pm.HalfNormal(
            "sigma", sigma=10, dims=("condition", "time", "switch_indicator")
        )
        ...
        rt_obs = pm.Normal(
            ...
            mu=mu[g, t, s],
            sigma=sigma[g, t, s],
            observed=df_transformed["results_rt"],
        )
        if len(coords["condition"]) > 1:
            # Deterministic node for switching cost
            switching_cost = pm.Deterministic(
                "switching_cost", mu[:, :, 1] - mu[:, :, 0]
            )
            # Deterministic parameters for the differences
            delta_zpdes = pm.Deterministic(
                "delta_zpdes", switching_cost[1, 1] - switching_cost[1, 0]
            )  # post - pre for zpdes
            delta_baseline = pm.Deterministic(
                "delta_baseline", switching_cost[0, 1] - switching_cost[0, 0]
            )  # post - pre for baseline
            delta_groups = pm.Deterministic(
                "delta_groups", delta_zpdes - delta_baseline
            )
            ...
            delta = pm.Deterministic(
                "delta", switching_cost[0, 1] - switching_cost[0, 0]
            )
...
def build_switching_cost_RT_hyper(data_combined, task_condition, coords):
    ...
    with pm.Model(coords=coords) as switching_cost_RT:
        ...
        mu = pm.Normal(
            ...
            mu=mu_hyper_mean,
            sigma=mu_hyper_sd,
            dims=("condition", "time", "switch_indicator"),
        )
        # Priors for group standard deviations given the hyperpriors
        sigma = pm.HalfNormal(
            "sigma",
            sigma=sigma_hyper_sd,
            dims=("condition", "time", "switch_indicator"),
        )
        # Likelihood for each observation
        rt_obs = pm.Normal(
...
def build_sdt_normal(data_combined, task_condition, coords):
    with pm.Model(coords=coords) as RT_model:
        ...
        if len(coords["condition"]) > 1:
            delta_zpdes = pm.Deterministic(
                "delta_zpdes", mu_d_prime[1, 1] - mu_d_prime[1, 0]
            )  # post - pre for zpdes
            delta_baseline = pm.Deterministic(
                "delta_baseline", mu_d_prime[0, 1] - mu_d_prime[0, 0]
            )  # post - pre for baseline
            delta_groups = pm.Deterministic(
                "delta_groups", delta_zpdes - delta_baseline
            )
            ...
            delta = pm.Deterministic(
                "delta", mu_d_prime[0, 1] - mu_d_prime[0, 0]
            )  # post - pre for zpdes
        # likelihood
        pm.Normal(
            "y",
            mu=mu_d_prime[g, t],
            sigma=sigma[g, t],
            observed=data_combined[f"{task_condition}-dprime"],
            dims="obs_id",
        )
        # pm.LogNormal("y", mu=mu[g, t], sigma=sigma[g, t],
        #              observed=data_combined[f'{task_condition}-rt'], dims="obs_id")
