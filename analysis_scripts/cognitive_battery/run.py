...
from analysis_scripts.cognitive_battery.preprocessing.preprocessing import (
    pre_process_all,
)

from analysis_scripts.cognitive_battery.hierarchical_bayesian_models.fit_models import (
    get_traces,
)
from analysis_scripts.cognitive_battery.hierarchical_bayesian_models.visualize import (
    plot_traces_and_deltas,
)
from analysis_scripts.cognitive_battery.hierarchical_bayesian_models.report import (
    get_latex_tables,
)
...
from analysis_scripts.cognitive_battery.aggregate_score.compute_aggregate import (
    run_broad_aggregate_difference,
)

# Define on what studies you want to conduct analysis
studies = ["v3_prolific"]

config_conditions = "main_config_model_fit.JSON"
# config_conditions = "v3_utl_only_mot.JSON"
config_fitting_params = "config_models_fit.JSON"
config_figure = "config_figures.JSON"
# config_figure = "v3_utl_mot_only_figure.JSON"
...
# with open('analysis_scripts/cognitive_battery/hierarchical_bayesian_models/config/short_test.JSON',
#           'r') as f:
with open(
    f"analysis_scripts/cognitive_battery/hierarchical_bayesian_models/config/{config_conditions}",
    "r",
) as f:
...
with open(
    f"analysis_scripts/cognitive_battery/hierarchical_bayesian_models/config/{config_fitting_params}",
    "r",
) as f:
...
with open(
    f"analysis_scripts/cognitive_battery/hierarchical_bayesian_models/config/{config_figure}",
    "r",
) as f:
...
def run_hierarchical_bayesian_models(
    studies, pre_process=True, fit_models=True, get_plot=True, get_latex=True, seed=42
):
    chrono = Chrono()
    print(
    ...
    if fit_models:
        ...
        get_traces(
            studies,
            all_conditions,
            nb_samples=config_fit_models["nb_samples"],
...
def run_dimensionality_reduction_models(studies):
    ...
    global_expe_name = "probit-all-tasks-ordered-new-metrics"
    ...
    # These are specific parameters to plot the results
    with open(
        "analysis_scripts/cognitive_battery/PCA/config/conditions_update.JSON", "r"
    ) as f:
        conditions = json.load(f)
    tasks, tasks_nb, kept_columns = format_conditions(conditions)
    n_components = len(tasks)
    models = ["PCA", "ICA"]
    ...
    run_PCA(
        ...
        tasks_nb,
        kept_columns,
        study_diff=studies[0],
        models=models,
        n_components=n_components,
        add_title="new_conditions",
    )

    # Run old and new config conditions:
    # These are specific parameters to plot the results
    with open(
        "analysis_scripts/cognitive_battery/PCA/config/conditions.JSON", "r"
    ) as f:
        conditions = json.load(f)
    tasks, tasks_nb, kept_columns = format_conditions(conditions)
    n_components = len(tasks)
